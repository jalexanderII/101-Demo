...
function formatMarketCap(value?: number): string {
  ...
  if (value >= trillion) {
    formatted = (value / trillion).toFixed(2);
    suffix = "T";
  } else if (value >= billion) {
    formatted = (value / billion).toFixed(2);
    suffix = "B";
  } else if (value >= million) {
    formatted = (value / million).toFixed(2);
    suffix = "M";
  } else {
  ...
  return trimmedDecimal 
    ? `$${formattedWhole}.${trimmedDecimal}${suffix}`
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <InfoItem
              icon={DollarSign}
              label="Market Cap"
              value={formatMarketCap(results.market_cap)}
            />
            <InfoItem
              icon={Briefcase}
              label="Exchange"
          ...
          <CardContent className="space-y-3">
            ...
            {results.cik && (
              <InfoItem
                icon={Hash}
                label="CIK"
                value={<code className="text-xs">{results.cik}</code>}
              />
            )}
            {results.sic_code && (
              <InfoItem
                icon={FileText}
                label="SIC Code"
                value={<code className="text-xs">{results.sic_code}</code>}
              />
            )}
            {results.locale && (
              <InfoItem
              ...
              <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
                ...
                {results.composite_figi && (
                  <InfoItem
                    ...
                    value={<code className="text-xs">{results.composite_figi}</code>}
                  />
                )}
                {results.share_class_figi && (
                  <InfoItem
                    ...
                    value={<code className="text-xs">{results.share_class_figi}</code>}
                  />
                )}
              </div>
            </CardContent>
