...
export function Financials({ results }: { results: FinancialsResult[] }) {
	...
	const INCOME_STATEMENT_STRUCTURE: IncomeStatementSection[] = [
			...
			items: [
				{
					key: "revenues",
					label: "Total Revenue",
					altKeys: ["total_revenue", "net_revenues", "revenue"],
					bold: true,
				},
				{
					key: "cost_of_revenue",
					label: "Cost of Revenue",
					altKeys: ["cost_of_goods_sold", "cost_of_sales"],
				},
				{
					key: "gross_profit",
					label: "Gross Profit",
			...
			items: [
				{
					...
					altKeys: ["research_development", "rd_expenses"],
				},
				{
				...
				},
				{
					...
					altKeys: ["total_operating_expenses"],
					bold: true,
				},
			],
		},
		{
			...
			items: [
				{
					key: "operating_income_loss",
					label: "Operating Income",
					altKeys: ["operating_income", "income_from_operations"],
					isSubtotal: true,
					bold: true,
				},
			],
		},
		{
			...
			items: [
				{
					key: "interest_income_expense_after_provision_for_losses",
				...
				{
					key: "other_than_temporary_impairment_losses_investments_available_for_sale_securities",
					label: "Investment Losses",
					altKeys: ["investment_losses"],
				},
				{
					key: "nonoperating_income_loss",
					label: "Other Income (Expense)",
					altKeys: ["other_income", "non_operating_income"],
				},
			],
		},
		{
			...
			items: [
				{
					key: "income_loss_from_continuing_operations_before_tax",
					label: "Income Before Taxes",
					altKeys: ["pretax_income", "earnings_before_taxes"],
					isSubtotal: true,
					bold: true,
				},
				{
					...
					altKeys: ["tax_expense", "provision_for_taxes"],
				},
				{
					key: "net_income_loss",
					label: "Net Income",
					altKeys: ["net_income", "net_earnings"],
					isSubtotal: true,
					bold: true,
				},
			],
			...
			items: [
				{
					...
					label: "Basic EPS",
					altKeys: ["earnings_per_share_basic", "basic_eps"],
				},
				{
					key: "diluted_earnings_per_share",
					label: "Diluted EPS",
					altKeys: ["earnings_per_share_diluted", "diluted_eps"],
				},
				{
				...
				{
					key: "weighted_average_shares_diluted",
	...
	function findFinancialPoint(
		statement: Statement,
		item: { key: string; altKeys?: string[] },
	): FinancialPoint | null {
					...
					<div className="grid gap-2 sm:grid-cols-2">
						{entries.map(([key, point]) => {
							...
							return (
								<div
									key={key}
									className="flex items-start justify-between gap-4 border-b py-1 last:border-b-0"
								>
	...
	return (
		<Tabs defaultValue="income" className="w-full">
			...
			<TabsContent value="income" className="mt-6">
				<Tabs
					defaultValue={results[0]?.fiscal_year || "2024"}
					className="w-full"
				>
					<TabsList className="grid w-full grid-cols-3">
						{results.map((period, idx) => (
							<TabsTrigger
								key={idx}
								value={period.fiscal_year || `year-${idx}`}
							>
					...
					{results.map((period, idx) => (
						...
						>
							<div className="text-xs text-muted-foreground mb-4">
								...
								{period.end_date
									? ` • End ${new Date(period.end_date).toLocaleDateString()}`
									: ""}
								{period.filing_date
									? ` • Filed ${new Date(period.filing_date).toLocaleDateString()}`
									: ""}
							</div>
